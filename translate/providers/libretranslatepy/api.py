...
#je vais essaye de rendre le script compatible avec python2 en modifiant les lignes def
    ...
    def translate(self, q, source = "en", target = "es", timeout = None):
        """Translate string
        ...
        #je modifie la ligne suivante aussi pour compatibilite python 2
        params = {"q": q, "source": source, "target": target}
        if self.api_key is not None:
            params["api_key"] = self.api_key
        url_params = urllib.parse.urlencode(params)
        req = urllib.request.Request(url, data=url_params.encode())
        response = urllib.request.urlopen(req, timeout = timeout)
        response_str = response.read().decode()
        return json.loads(response_str)["translatedText"]

    def detect(self, q, timeout = None):
    ...
    def languages(self, timeout = None):
        ...
        if self.api_key is not None:
            params["api_key"] = self.api_key
        url_params = urllib.parse.urlencode(params)
        req = urllib.request.Request(url, data=url_params.encode(), method="GET")
        response = urllib.request.urlopen(req, timeout = timeout)
        response_str = response.read().decode()
        return json.loads(response_str)
